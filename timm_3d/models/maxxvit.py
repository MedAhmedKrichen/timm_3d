...
from timm_3d.layers import Mlp, ConvMlp, DropPath, LayerNorm, ClassifierHead, NormMlpClassifierHead
from timm_3d.layers import create_attn, get_act_layer, get_norm_layer, get_norm_act_layer, create_conv3d, create_pool3d
from timm_3d.layers import trunc_normal_tf_, to_3tuple, extend_tuple, make_divisible, _assert
from timm_3d.layers import RelPosMlp, RelPosBias, RelPosBiasTf, use_fused_attn, resize_rel_pos_bias_table
...
class MaxxVitTransformerCfg:
    ...
    act_layer: str = 'gelu'
    norm_layer: str = 'layernorm3d'
    norm_layer_cl: str = 'layernorm'
    norm_eps: float = 1e-6

    def __post_init__(self):
        if self.grid_size is not None:
            self.grid_size = to_3tuple(self.grid_size)
        if self.window_size is not None:
            self.window_size = to_3tuple(self.window_size)
            if self.grid_size is None:
                self.grid_size = self.window_size
        # print("ZZZ", self.window_size, self.grid_size)
...
class MaxxVitConvCfg:
    ...
    def __post_init__(self):
        ...
        if not self.norm_layer:
            self.norm_layer = 'batchnorm3d' if use_mbconv else 'layernorm3d'
...
class Attention3d(nn.Module):
    ...
    """ multi-head attention for 3D NCHW tensors"""
    def __init__(
            self,
            dim: int,
        ...
        self.qkv = nn.Conv3d(dim, dim_attn * 3, 1, bias=bias)
        self.rel_pos = rel_pos_cls(num_heads=self.num_heads) if rel_pos_cls else None
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Conv3d(dim_attn, dim_out, 1, bias=bias)
    ...
    def forward(self, x, shared_rel_pos: Optional[torch.Tensor] = None):
        ...
        if self.fused_attn:
            ...
            # print("???", x.shape, q.shape, k.shape, v.shape, attn_bias.shape)
            ...
            x = (v @ attn.transpose(-2, -1)).view(B, -1, H, W, D)
...
class AttentionCl(nn.Module):
    ...
    def forward(self, x, shared_rel_pos: Optional[torch.Tensor] = None):
        ...
        if self.head_first:
            # print("!!!", x.shape, self.dim, self.dim_attn)
            tmp = self.qkv(x)
            # print("!!!!", tmp.shape)
            q, k, v = tmp.view(B, -1, self.num_heads, self.dim_head * 3).transpose(1, 2).chunk(3, dim=3)
        else:
            # print("!!!v2", x.shape, self.dim, self.dim_attn, self.dim_out)
            tmp = self.qkv(x)
            # print("!!!!v2", tmp.shape)
            tmp2 = tmp.reshape(B, -1, 3, self.num_heads, self.dim_head)
            # print("!!!!!v2", tmp2.shape)
            q, k, v = tmp2.transpose(1, 3).unbind(2)
            # print("RRR1", q.shape, k.shape, v.shape)
        ...
        if self.fused_attn:
            ...
            # print("RRR", q.shape, k.shape, v.shape, attn_bias.shape)
            x = torch.nn.functional.scaled_dot_product_attention(
                q, k, v,
                attn_mask=attn_bias,
                dropout_p=self.attn_drop.p if self.training else 0.,
...
class Downsample3d(nn.Module):
    ...
    * 'avg2' - AvgPool2d w/ kernel_size = stride = 2
    ...
    def __init__(
        ...
        if pool_type == 'max':
            self.pool = create_pool3d('max', kernel_size=3, stride=2, padding=padding or 1)
        elif pool_type == 'max2':
            self.pool = create_pool3d('max', 2, padding=padding or 0)  # kernel_size == stride == 2
        elif pool_type == 'avg':
            self.pool = create_pool3d(
                'avg', kernel_size=3, stride=2, count_include_pad=False, padding=padding or 1)
        else:
            self.pool = create_pool3d('avg', 2, padding=padding or 0)

        if dim != dim_out:
            self.expand = nn.Conv3d(dim, dim_out, 1, bias=bias)
        else:
...
class TransformerBlock3d(nn.Module):
    """ Transformer block with 3D downsampling
    ...
    def __init__(
        ...
        if stride == 2:
            self.shortcut = Downsample3d(dim, dim_out, pool_type=cfg.pool_type, bias=cfg.shortcut_bias)
            self.norm1 = nn.Sequential(OrderedDict([
                ('norm', norm_layer(dim)),
                ('down', Downsample3d(dim, dim, pool_type=cfg.pool_type)),
            ]))
        else:
            assert dim == dim_out
        ...
        self.ls1 = LayerScale3d(dim_out, init_values=cfg.init_values) if cfg.init_values else nn.Identity()
        ...
        self.mlp = ConvMlp(
            ...
            act_layer=act_layer,
            drop=cfg.proj_drop)
        self.ls2 = LayerScale3d(dim_out, init_values=cfg.init_values) if cfg.init_values else nn.Identity()
...
class MbConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if stride_pool > 1:
            self.down = Downsample3d(in_chs, in_chs, pool_type=cfg.downsample_pool_type, padding=cfg.padding)
        else:
            self.down = nn.Identity()
        self.conv1_1x1 = create_conv3d(in_chs, mid_chs, 1, stride=stride_1)
        self.norm1 = norm_act_layer(mid_chs)

        self.conv2_kxk = create_conv3d(
        ...
        self.conv3_1x1 = create_conv3d(mid_chs, out_chs, 1, bias=cfg.output_bias)
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        if stride == 2:
            self.shortcut = Downsample3d(in_chs, out_chs)
        elif in_chs != out_chs:
            self.shortcut = nn.Conv3d(in_chs, out_chs, kernel_size=1, bias=cfg.output_bias)
        else:
            self.shortcut = nn.Identity()
        ...
        if stride_pool == 2:
            self.down = Downsample3d(in_chs, in_chs, pool_type=cfg.downsample_pool_type)
        else:
            self.down = nn.Identity()

        self.conv_dw = create_conv3d(
        ...
        if conv_mlp:
            self.ls = LayerScale3d(out_chs, cfg.init_values) if cfg.init_values else nn.Identity()
        else:
...
def window_reverse(windows, window_size: List[int], img_size: List[int]):
    H, W, D = img_size
    C = windows.shape[-1]
    x = windows.view(-1, H // window_size[0], W // window_size[1], D // window_size[2], window_size[0], window_size[1], window_size[2], C)
...
def grid_partition(x, grid_size: List[int]):
    ...
    _assert(D % grid_size[2] == 0, '')
    x = x.view(B, grid_size[0], H // grid_size[0], grid_size[1], W // grid_size[1], grid_size[2], D // grid_size[2], C)
    windows = x.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous().view(-1, grid_size[0], grid_size[1], grid_size[2], C)
    return windows
...
def grid_reverse(windows, grid_size: List[int], img_size: List[int]):
    H, W, D = img_size
    C = windows.shape[-1]
    x = windows.view(-1, H // grid_size[0], W // grid_size[1], D // grid_size[2], grid_size[0], grid_size[1], grid_size[2], C)
    x = x.permute(0, 4, 1, 5, 2, 6, 3, 7).contiguous().view(-1, H, W, D, C)
    return x
...
class PartitionAttentionCl(nn.Module):
    ...
    def __init__(
        ...
        self.partition_size = to_3tuple(cfg.window_size if self.partition_block else cfg.grid_size)
    ...
    def _partition_attn(self, x):
        img_size = x.shape[1:4]
        if self.partition_block:
            # print('W part', img_size)
            partitioned = window_partition(x, self.partition_size)
            # print(partitioned.shape)
        else:
            # print('G part', img_size, self.partition_size)
    ...
    def forward(self, x):
        ...
        tmp = self.norm1(x)
        # print("!K!", tmp.shape)
        tmp2 = self._partition_attn(tmp)
        # print("!L!", tmp2.shape)
        x = x + self.drop_path1(self.ls1(tmp2))
        # print("!TT!", x.shape)
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        # print("!TTT!", x.shape)
        return x
...
class ParallelPartitionAttention(nn.Module):
    ...
    def __init__(
        ...
        self.partition_size = to_3tuple(cfg.window_size)
        rel_pos_cls = get_rel_pos_cls(cfg, self.partition_size)
...
def window_partition_nchw(x, window_size: List[int]):
    ...
    _assert(W % window_size[1] == 0, '')
    _assert(D % window_size[2] == 0, '')
    x = x.view(B, C, H // window_size[0], window_size[0], W // window_size[1], window_size[1], D // window_size[2], window_size[2])
    windows = x.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous().view(-1, C, window_size[0], window_size[1], window_size[2])
    return windows
...
def window_reverse_nchw(windows, window_size: List[int], img_size: List[int]):
    H, W, D = img_size
    C = windows.shape[1]
    x = windows.view(-1, H // window_size[0], W // window_size[1], D // window_size[2], C, window_size[0], window_size[1], window_size[2])
    x = x.permute(0, 4, 1, 5, 2, 6, 3, 7).contiguous().view(-1, C, H, W, D)
    return x
...
def grid_partition_nchw(x, grid_size: List[int]):
    ...
    _assert(W % grid_size[1] == 0, '')
    _assert(D % grid_size[2] == 0, '')
    x = x.view(B, C, grid_size[0], H // grid_size[0], grid_size[1], W // grid_size[1], grid_size[2], D // grid_size[2])
    windows = x.permute(0, 3, 5, 7, 1, 2, 4, 6).contiguous().view(-1, C, grid_size[0], grid_size[1], grid_size[2])
    return windows
...
def grid_reverse_nchw(windows, grid_size: List[int], img_size: List[int]):
    H, W, D = img_size
    C = windows.shape[1]
    x = windows.view(-1, H // grid_size[0], W // grid_size[1], D // grid_size[2], C, grid_size[0], grid_size[1], grid_size[2])
    x = x.permute(0, 3, 5, 1, 6, 2, 7, 3).contiguous().view(-1, C, H, W, D)
    return x


class PartitionAttention3d(nn.Module):
    """ Grid or Block partition + Attn + FFN
    ...
    def __init__(
        ...
        self.ls1 = LayerScale3d(dim, init_values=cfg.init_values) if cfg.init_values else nn.Identity()
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        ...
        self.mlp = ConvMlp(
            ...
            act_layer=act_layer,
            drop=cfg.proj_drop)
        self.ls2 = LayerScale3d(dim, init_values=cfg.init_values) if cfg.init_values else nn.Identity()
        self.drop_path2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class MaxxVitBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.nchwd_attn = transformer_cfg.use_nchw_attn
        ...
        partition_layer = PartitionAttention3d if self.nchwd_attn else PartitionAttentionCl
    ...
    def forward(self, x):
        ...
        if not self.nchwd_attn:
            x = x.permute(0, 2, 3, 4, 1)  # to NHWDC (channels-last)
        if self.attn_block is not None:
            x = self.attn_block(x)
        x = self.attn_grid(x)
        if not self.nchwd_attn:
            x = x.permute(0, 4, 1, 2, 3)  # back to NCHWD
        return x
...
class MaxxVitStage(nn.Module):
    def __init__(
            ...
            stride: int = 2,
            depth: int = 4,
            feat_size: Tuple[int, int] = (14, 14, 14),
...
class Stem(nn.Module):
    ...
    def __init__(
        ...
        # print("EEE", padding, bias, kernel_size, in_chs, out_chs[0])
        self.conv1 = create_conv3d(in_chs, out_chs[0], kernel_size, stride=2, padding=padding, bias=bias)
        self.norm1 = norm_act_layer(out_chs[0])
        self.conv2 = create_conv3d(out_chs[0], out_chs[1], kernel_size, stride=1, padding=padding, bias=bias)
...
def cfg_window_size(cfg: MaxxVitTransformerCfg, img_size: Tuple[int, int, int]):
    if cfg.window_size is not None:
        assert cfg.grid_size
        return cfg
    partition_size = img_size[0] // cfg.partition_ratio, img_size[1] // cfg.partition_ratio, img_size[2] // cfg.partition_ratio
...
class MaxxVit(nn.Module):
    ...
    def __init__(
        ...
        feat_size = tuple([i // s for i, s in zip(img_size, to_3tuple(stride))])
...
def _rw_coat_cfg(
        ...
        transformer_norm_layer='layernorm3d',
...
def _next_cfg(
    ...
    init_values = to_3tuple(init_values)
    return dict(
        conv_cfg=MaxxVitConvCfg(
            block_type='convnext',
            stride_mode=stride_mode,
...
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 1000, 'input_size': (3, 128, 128, 128), 'pool_size': (7, 7, 7),
...
default_cfgs = generate_default_cfgs({
    ...
    'coatnet_rmlp_2_rw_384.sw_in12k_ft_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 192, 192, 192), pool_size=(12, 12), crop_pct=1.0, crop_mode='squash'),
    ...
    'maxvit_pico_rw_256.untrained': _cfg(url='', input_size=(3, 256, 256, 256), pool_size=(8, 8, 8)),
    'maxvit_nano_rw_256.sw_in1k': _cfg(
        ...
        input_size=(3, 256, 256, 256), pool_size=(8, 8, 8)),
    ...
    'maxvit_tiny_rw_256.untrained': _cfg(
        url='',
        input_size=(3, 256, 256, 256), pool_size=(8, 8, 8)),
    'maxvit_tiny_pm_256.untrained': _cfg(url='', input_size=(3, 256, 256, 256), pool_size=(8, 8, 8)),
    ...
    'maxvit_rmlp_base_rw_384.sw_in12k_ft_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 128, 128, 128), pool_size=(12, 12, 12), crop_pct=1.0, crop_mode='squash'),

    # timm specific MaxVit w/ ImageNet-12k pretrain
    'maxvit_rmlp_base_rw_224.sw_in12k': _cfg(
        hf_hub_id='timm/',
        num_classes=11821,
        input_size=(3, 128, 128, 384)
    ),
    ...
    'maxxvit_rmlp_nano_rw_256.sw_in1k': _cfg(
        ...
        input_size=(3, 256, 256, 256), pool_size=(8, 8, 8)),
    'maxxvit_rmlp_tiny_rw_256.untrained': _cfg(url='', input_size=(3, 256, 256, 256), pool_size=(8, 8, 8)),
    'maxxvit_rmlp_small_rw_256.sw_in1k': _cfg(
        ...
        input_size=(3, 128, 256, 256), pool_size=(8, 8, 8)),
    ...
    'maxxvitv2_rmlp_base_rw_384.sw_in12k_ft_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 384, 384, 384), pool_size=(12, 12, 12), crop_pct=1.0, crop_mode='squash'),
    ...
    'maxvit_tiny_tf_224.in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 64, 64, 64),
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD),
    'maxvit_tiny_tf_384.in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 96, 96, 96), pool_size=(3, 3, 3), crop_pct=1.0, crop_mode='squash'),
    'maxvit_tiny_tf_512.in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 128, 128, 128), pool_size=(4, 4, 4), crop_pct=1.0, crop_mode='squash'),
    ...
    'maxvit_base_tf_512.in21k_ft_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 512, 512), pool_size=(4, 4, 4), crop_pct=1.0, crop_mode='squash'),
    ...
    'maxvit_large_tf_512.in21k_ft_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(3, 128, 128, 128), crop_pct=1.0, crop_mode='squash'),
